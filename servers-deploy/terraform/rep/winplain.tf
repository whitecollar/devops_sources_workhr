...
resource "vcd_vapp" "win16dev" {
  ...
  provisioner "local-exec" {
    command = "echo ${self.ip} ${self.name} "
}
...
provisioner "local-exec" {
//    when                  = "destroy"
   // command = "sshpass -p \"${var.pw}\" ssh -o StrictHostKeyChecking=no ${var.wuser}@${var.ip_rmq} \"powershell.exe -ExecutionPolicy Bypass -File C:\\run.ps1\" "
  }
