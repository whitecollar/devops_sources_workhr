...
provisioner "file" {
  source      = "../../../software-postinstall/sql-server.ps1"
...
connection {
  ...
  provisioner "local-exec" {
     ...
     sleep 100
     sshpass -p  '${var.adminPassword}' ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${var.rootAdminLogin}@${self.ip} 'powershell.exe -ExecutionPolicy Bypass -File C:\\postinstall.ps1 ${var.saPassword}'
     EOT
  }
