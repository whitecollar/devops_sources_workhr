                ...
                timestamps {
                    script {
                        ...
                        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'exch-setup',
                            ...
                            returnCode = utils.cmd("powershell -file ${env.WORKSPACE}/maintenance/exchange_enable_mailbox.ps1 ${BITConvJava.EXCHANGE_URL}/powershell/ ${user} ${Email} ${USERNAME} ${PASSWORD}")
                        }
                        if (returnCode != 0) {
                            utils.raiseError("Почтовый ящик не создался")
                        }
                    }
                }
            }
